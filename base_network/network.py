...
class GRU:
    def __init__(self,is_training,word_embeddings,settings):
        ...
        inputs_backward = tf.concat([tf.nn.embedding_lookup(word_embedding,tf.reverse(self.input_word,[False,True])),tf.nn.embedding_lookup(pos1_embedding,tf.reverse(self.input_pos1,[False,True])),tf.nn.embedding_lookup(pos1_embedding,tf.reverse(self.input_pos2,[False,True]))],2)
        ...
        for i in range(big_num):
            ...
            self.sen_alpha.append(tf.reshape(tf.nn.softmax(tf.reshape(tf.matmul(tf.multiply(sen_repre[i],sen_a),sen_r),[batch_size])),[1,batch_size]))
            
            #self.attention.append(sen_alpha)
    
            self.sen_s.append(tf.reshape(tf.matmul(self.sen_alpha[i],sen_repre[i]),[gru_size,1]))
            sen_out.append(tf.add(tf.reshape(tf.matmul(relation_embedding,self.sen_s[i]),[self.num_classes]),sen_d))
            ...
            self.max_prob = tf.reduce_max(self.prob[i])
            self.class_entropy = -tf.reduce_sum(self.prob[i]*tf.log(self.prob[i]))

            with tf.name_scope("loss"):
                self.loss.append(tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=sen_out[i], labels=self.input_y[i])))
                if i == 0:
                    self.total_loss = self.loss[i]
                else:
                    self.total_loss += self.loss[i]
            self.loglikelihood = -self.total_loss
