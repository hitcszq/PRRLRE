...
class PolicyGradientREINFORCE(object):
  ...
  def updateModel(self):
    ...
    for t in range(N-1):
      ...
      states  = np.array(self.state_buffer[t])
  ...
  def sampleActiontest(self,states):
    self.exploration = 0
    return self.sampleAction(states)
...
class PRPolicyGradientREINFORCE(PolicyGradientREINFORCE):
  def __init__(self, session,
                     optimizer,
                     policy_network,
                     state_dim,
    ...
    super(PRPolicyGradientREINFORCE, self).__init__(session,
                         optimizer,
                         policy_network,
                         state_dim,
                         num_actions,
  ...
  def sampleAction(self, states):
    ...
    # epsilon-greedy exploration strategy
    distr = np.array([1.0/math.e,1.0])

    if random.random() < self.exploration:
      return random.randint(0, self.num_actions-1)
    else:
      action_scores = self.session.run(self.action_scores, {self.states: state})[0]
      action_probs  = softmax(action_scores) - 1e-5
      if ind == 1:
        action_probs *= distr
        action_probs  = softmax(action_probs) - 1e-5
      action = np.argmax(np.random.multinomial(1, action_probs))
      return action
