...
def train_GRU(agent):
	...
	train_word = np.load('./data/train_word_new.npy')
	train_pos1 = np.load('./data/train_pos1_new.npy')
	train_pos2 = np.load('./data/train_pos2_new.npy')
	...
	with tf.Graph().as_default():
		...
		with sess.as_default():
			...
			def train_step(word_batch, pos1_batch, pos2_batch, y_batch,big_num):
				...
				temp, step, loss, accuracy,l2_loss,final_loss= sess.run([train_op, global_step, m.total_loss, m.accuracy,m.l2_loss,m.final_loss], feed_dict)
			...
			for one_epoch in range(settings.num_epochs):
				...
				for i in range(int(len(temp_order)/float(settings.big_num))):
					...
					if current_step > 2000 and current_step%500==0:
					#if current_step == 50:
						print 'saving model'
						path = saver.save(sess,save_path +'%sATT_GRU_model'%agent,global_step=current_step)
